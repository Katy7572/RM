...
# data.js 路径：优先脚本同级目录，不存在则用上一级（适配本地和 GitHub Actions 两种目录结构）
...
def fetch(codes):
    def prefix(c):
        if c[0] in '48': return 'bj' + c       # 北交所 8xx/4xx
        if c.startswith('92'): return 'bj' + c  # 北交所 920xxx
        if c[0] in '69': return 'sh' + c       # 上交所 6xx
        return 'sz' + c                         # 深交所 0xx/3xx
    prefixed = ','.join([prefix(c) for c in codes])
    try:
        ...
        for line in resp.text.strip().split(';'):
            ...
            code = parts[0].replace('v_','').replace('sh','').replace('sz','').replace('bj','')
